...
const isExpired = computed(() => Boolean(run.value) && remainingMs.value <= 0)
...
const timerLabel = computed(() => formatRemaining(remainingMs.value))
const timerState = computed(() => {
  if (isExpired.value) return 'expired'
  if (remainingMs.value <= 60 * 1000) return 'warning'
  return ''
})
...
async function answer(guess: Guess) {
  const question = currentQuestion.value
  if (!run.value || !question || locked.value || isExpired.value) return
  locked.value = true
  flushError.value = ''
...
function tickTimer() {
  if (!run.value) return
  remainingMs.value = Math.max(0, run.value.resumeExpiresAt - Date.now())
  if (remainingMs.value === 0) {
    locked.value = true
    flushError.value = 'The session seal has closed. Convene a new council to begin again.'
    game.clearRun()
    if (timerId) {
...
<template>
  <main id="main" class="court quiz-court">
    ...
    <section v-if="run && currentQuestion" class="quiz-shell">
      ...
      <div class="session-timer" :class="timerState" aria-live="polite">
        <span>{{ isExpired ? 'Session seal closed' : 'Session seal closes in' }}</span>
      ...
      <div class="bench">
        <button class="verdict ai" :disabled="locked || isExpired" @click="answer('ai')">
          <span class="big">Machine</span>
          <span class="desc">generated by AI</span>
          <span class="key">Press A</span>
        </button>
        <button class="verdict human" :disabled="locked || isExpired" @click="answer('human')">
